...
public class TradeConfigScreen extends BaseOwoScreen<FlowLayout> {
    ...
    protected void build(FlowLayout rootComponent) {
        ...
        window.child(Components.label(Text.of("Bot Settings")).shadow(true).margins(Insets.bottom(10)));

        var itemShowcase = Components.item(InvisAuc.getTargetStack());
        window.child(itemShowcase.margins(Insets.bottom(8)));

        window.child(Components.button(Text.of("Read from Hand"), button -> {
        ...
        priceField.onChanged().subscribe(val -> {
            try { if (!val.isEmpty()) InvisAuc.setCurrentPrice(Integer.parseInt(val.trim())); } catch (NumberFormatException ignored) {}
        });
